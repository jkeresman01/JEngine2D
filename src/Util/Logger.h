...
#define JENGINE_DEBUG(message) LOG("DEBUG", message)
#define JENGINE_INFO(message) LOG("INFO", message)
#define JENGINE_WARN(message) LOG("WARN", message)
#define JENGINE_ERROR(message) LOG("ERROR", message)
...
class Logger
{
  public:
    ...
    static void log(const std::string &severity, const std::string &message, const char *file,
                    uint32_t lineNumber)
    {
        std::ofstream logfile("C:\\Users\\josip\\aaaaa\\JEngine2D\\log.txt", std::ios::app);
